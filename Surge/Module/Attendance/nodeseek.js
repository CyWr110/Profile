...
[Script]
http-response ^https:\/\/www\.nodeseek\.com\/api\/account\/getInfo\/.+\?readme= script-path=https://gist.githubusercontent.com/Sliverkiss/2b5acc2c4960dd06618c6912302c2c7f/raw/nodeseek.js, requires-body=true, timeout=60, tag=NodeSeek获取token
...
class UserInfo {
    ...
    async userAccount() {
        try {
            let res = await this.fetch(`/api/account/getInfo/${this.userId}?readme=1`);
            return res?.detail;
        } catch (e) {
            this.ckStatus = false;
